...
class _AtrousSpatialPyramidPoolingModule_no_edge(nn.Module):
    '''
    ...
    def __init__(self, in_dim, reduction_dim=256, output_stride=16, rates=[6, 12, 18]):
        super(_AtrousSpatialPyramidPoolingModule_no_edge, self).__init__()

        # Check if we are using distributed BN and use the nn from encoding.nn
        # library rather than using standard pytorch.nn

        if output_stride == 8 or output_stride == 2:
...
class Disentangler_v0(nn.Module):
    def __init__(self, cin):
        super(Disentangler_v0, self).__init__()

    def forward(self, x, mask):
        N, C, H, W = x.size()
        
        mask_ = mask.reshape(N, 1, H * W)                          # [N, 1, H*W]
        x = x.reshape(N, C, H * W).permute(0, 2, 1).contiguous()   # [N, H*W, C]
        fg_feats = torch.matmul(mask_, x) / (H * W)                # [N, 1, C]
        bg_feats = torch.matmul(1 - mask_, x) / (H * W)            # [N, 1, C]
...
class Disentangler(nn.Module):
    ...
    def forward(self, fts, mask):
        fts = F.interpolate(fts, size=mask.shape[-2:], mode='bilinear')
        masked_fg_feats = torch.sum(fts * mask, dim=(2, 3)) \
            / (mask.sum(dim=(2, 3)) + 1e-5)  # N x C

        masked_bg_feats = torch.sum(fts * mask, dim=(2, 3)) \
            / (mask.sum(dim=(2, 3)) + 1e-5)  # N x C
        return masked_fg_feats,masked_bg_feats
...
class Back_PVT(nn.Module):
    def __init__(self, channel=32):
        super(Back_PVT, self).__init__()
        self.backbone = pvt_v2_b2()  # [64, 128, 320, 512]

        path = './preTrainModel/pvt_v2_b2.pth'
        ...
        self.edge_layer = Edge_Module(in_fea=[64, 128, 512])
        ...
        self.out_cross = nn.Conv2d(channel, 1, 1)
        self.out_cfm = nn.Conv2d(channel, 1, 1)
        self.down05 = nn.Upsample(scale_factor=0.5, mode='bilinear', align_corners=True)

        self.attention1 = Self_Mask_Attention(dim=32)
        self.attention2 = Self_Mask_Attention(dim=32)

        # self.disentangler = Disentangler(cin=32)
        self.disentangler = Disentangler_v0(cin=32)

        self.ca1 = ChannelAttention(64)
        self.sa1 = SpatialAttention()
        self.ca2 = ChannelAttention(64)
        self.sa2 = SpatialAttention()
        self.aspp_x1 = _AtrousSpatialPyramidPoolingModule_no_edge(64,32,output_stride=2)
        self.after_aspp_x1 = nn.Conv2d(160, 64, kernel_size=1, bias=False)
        self.conv_x1 = nn.Conv2d(128, 64, 1)
    ...
    def forward(self, x):
        ...
        x1_copy_1 = x1 # [1, 64, 88, 88]
        x1_copy_2 = x1 # [1, 64, 88, 88]

        x1_copy_1 = self.aspp_x1(x1_copy_1)
        x1_copy_1 = self.after_aspp_x1(x1_copy_1)

        x1_fused = self.relu(torch.cat((x1_copy_1, x1_copy_2), 1))
        x1_fused = self.conv_x1(x1_fused)

        ca1 = self.ca2(x1_copy_1)
        sa1 = self.sa2(x1_copy_1)

        ca2 = self.ca2(x1_copy_2)
        sa2 = self.sa2(x1_copy_2)

        ca_fused = torch.mul(ca1,ca2)
        sa_fused = torch.mul(sa1,sa2)

        cim_feature = sa_fused * (ca_fused * x1_fused)
        
        # CFM
        x2_t = self.Translayer2_1(x2)  
        x3_t = self.Translayer3_1(x3)  
        x4_t = self.Translayer4_1(x4)  
        cfm_feature = self.CFM(x4_t, x3_t, x2_t) # cfm_feature:[1, 32, 44, 44]

        # T2
        T2 = self.Translayer2_0(cim_feature)  # T2:[1, 32, 88, 88]
        T2 = self.down05(T2) # [1, 32, 44, 44]

        # edge_map = self.edge_layer(x1, x3, x4) #in_fea=[64, 256, 512]
        edge_map = self.edge_layer(x1, x2, x4) #in_fea=[64, 128, 512]
        edge_map_small = F.interpolate(edge_map, scale_factor=0.5, mode='bilinear', align_corners=False) # edge_map_small:[1, 1, 44, 44]
        edge_map = F.interpolate(edge_map, scale_factor=4, mode='bilinear', align_corners=False)  
        edge_out = torch.sigmoid(edge_map)

        prediction1 = self.out_cfm(cfm_feature)  # init_sal
        cfm_feature_attention = self.attention1(cfm_feature,prediction1)
        T2_attention = self.attention2(T2,edge_map_small)

        cross_feature2 = self.cross_attention(cfm_feature_attention, T2_attention) # v8
        prediction = self.out_cross(cross_feature2)  
        prediction_8 = F.interpolate(prediction, scale_factor=8, mode='bilinear')  

        sal_feature = self.sal_conv(prediction_8) # v5
        edge_feature = self.edge_conv(edge_map) # [1, 32, 352, 352]
        ...
        fg_feats, bg_feats = self.disentangler(sal_edge_feature,sal_ref)
        # return prediction_8, edge_map, sal_ref
        return prediction_8, edge_map, sal_ref, fg_feats, bg_feats

if __name__ == '__main__':
    model = Back_PVT().cuda()
    input_tensor = torch.randn(1, 3, 352, 352).cuda()

    # prediction1, prediction2,edge = model(input_tensor)
    # print(prediction1[2].size(), prediction2.size())
    # x1,x2,x3,x4,x5 = model(input_tensor)
